...
class NextActionAgent:
    ...
    def _get_channel_recommendation(self, lead_data: LeadInput, heat_score: HeatScore) -> Channel:
        """Determine recommended channel - all leads use email with RAG personalization."""
        # All leads use email with RAG personalization
    ...
    def _get_fallback_recommendation(
        ...
        """Generate dynamic fallback recommendation based on real lead data."""
        channel = self._get_channel_recommendation(lead_data, heat_score)
        
        # Extract real lead values for personalization
        lead_name = lead_data.name or "Valued Professional"
        lead_role = lead_data.role or "Professional"
        lead_campaign = lead_data.campaign or "our program"
        lead_company = getattr(lead_data, 'company', '') or ""
        lead_region = getattr(lead_data, 'region', '') or ""
        lead_source = getattr(lead_data, 'source', '') or ""
        lead_page_views = getattr(lead_data, 'page_views', 0) or 0
        lead_recency = getattr(lead_data, 'recency_days', 0) or 0
        lead_interest = lead_data.prior_course_interest or "medium"
        lead_keywords = lead_data.search_keywords or ""
        
        # Generate dynamic messages based on heat score and real data
        if heat_score == HeatScore.HOT:
            # Hot leads: Urgent, direct message with real data
            message = f"""Hi {lead_name}!

I noticed your strong engagement with our {lead_campaign.replace('_', ' ')} program - {lead_page_views} page views shows real interest! As a {lead_role} at {lead_company or 'your organization'}, this could be perfect for your career growth.

Based on your profile from {lead_source} and your {lead_interest} interest level, I'd like to offer you a personalized consultation to discuss how our program can help you achieve your goals.

Since it's been {lead_recency} days since your last interaction, I want to make sure you don't miss this opportunity.

Would you be available for a quick call this week?

Best regards,
Lead HeatScore Team"""
            rationale = f"Hot lead - High engagement ({lead_page_views} views), recent activity ({lead_recency} days), personalized outreach"
            
        elif heat_score == HeatScore.WARM:
            # Warm leads: Nurturing message with real data
            message = f"""Hi {lead_name}!

Thank you for your interest in our {lead_campaign.replace('_', ' ')} program. As a {lead_role} in {lead_region or 'your region'}, I believe you'd find our resources particularly valuable.

I noticed you came to us through {lead_source} and have shown {lead_interest} interest level. Based on your search keywords ({lead_keywords or 'your interests'}), I'd like to share some additional information that might be relevant to your goals.

Would you be interested in learning more about our personalized learning paths designed for {lead_role} professionals?

Best regards,
Lead HeatScore Team"""
            rationale = f"Warm lead - Moderate engagement ({lead_page_views} views), {lead_interest} interest, nurturing message"
            
        else:
            # Cold leads: Educational message with real data
            message = f"""Hi {lead_name}!

I hope you're doing well. I wanted to reach out regarding our {lead_campaign.replace('_', ' ')} program, which offers comprehensive learning paths designed for {lead_role} professionals like you.

I noticed you found us through {lead_source} and are interested in {lead_keywords or 'professional development'}. Since it's been {lead_recency} days since your last visit, I thought you might appreciate some valuable resources that could be relevant for your role.

Would you like to receive our educational newsletter with industry insights and learning opportunities?

Best regards,
Lead HeatScore Team"""
            rationale = f"Cold lead - Low engagement ({lead_page_views} views), {lead_recency} days recency, educational outreach"
...
"""
    ...
    async def craft_first_message(
        ...
        try:
            ...
            # Craft message based on channel - all leads use RAG email service
            # Use RAG email service for detailed personalization
            from app.services.langgraph_workflow import lead_workflow
            rag_email = await lead_workflow.process_lead(lead_data)
            message_content = rag_email["content"]
            channel = Channel.EMAIL
            
            # Create rationale for RAG email
