...
async def connect_to_mongo():
    ...
    try:
        ...
        # Create text search index for BM25-like scoring
...
async def create_text_index():
    ...
    try:
        ...
        indexes = await collection.list_indexes().to_list(length=None)
        text_index_exists = any('_fts' in str(idx) or idx.get('name') == 'text_index' for idx in indexes)
        
        if not text_index_exists:
            print("Creating text search index for BM25-like scoring...")
            # Create compound text index on title, content, and category
        ...
        print("BM25 search will fall back to regex matching")
